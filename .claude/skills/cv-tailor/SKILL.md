---
name: cv-tailor
description: Adapta un CV YA GENERADO (por cv-builder, en trabajo/cv/*.typ) a una Job Description específica, exagerando el fit dentro de lo defendible para maximizar el match con ese puesto. Reordena stack y bullets, reescribe el summary al rol de la oferta, refuerza las keywords de la JD y flaguea los gaps reales. Usá esta skill siempre que el alumno pegue una oferta/JD y quiera adaptar su CV a ese puesto, o diga "ajustá mi CV a esta oferta", "adaptá el CV a esta JD", "CV para esta búsqueda", "CV para este puesto", "tailoreá el CV", "hacé un CV para esta oferta", "/cv-tailor". Requiere un CV base ya validado — si no existe ninguno en trabajo/cv/, deriva a cv-builder primero. NO es para generar el CV desde cero (eso es cv-builder) ni para preparar la entrevista (eso es interview-prep). Requiere typst instalado. NO inventa empresas, roles ni años.
---
...
- **Un CV base en `trabajo/cv/*.typ`** (lo generó `cv-builder`). Si no hay ninguno → *"No encuentro un CV base tuyo. Generalo primero con `cv-builder` y después volvé con la oferta."* y parar. El CV base es el material validado; esta skill solo lo re-enfoca, no lo crea.
...
- Una **copia tailoreada** del `.typ` base en `trabajo/cv/{slug}-cv-{jd-slug}.typ` (+ PDF con el mismo nombre), sin tocar el base.
...
- **Límite con `cv-builder`:** `cv-builder` arma el CV **desde el perfil** (y opcionalmente lo tailorea *ligero* si le pasás una JD al momento de crearlo). `cv-tailor` asume que el base **ya existe y está validado**, y va **más fuerte** con el match: reescribe summary, reordena todo hacia la JD, refuerza keywords y empuja el fit.
...
- **Sibling:** `linkedin-profile-optimizer` — el CV tailoreado y el LinkedIn deben contar la misma historia.

## Step 1 — Validar CV base + typst

1. Listar los CV base disponibles:
   ```bash
   ls trabajo/cv/*.typ 2>/dev/null
   ```
   - **Ninguno** → derivar a `cv-builder` y parar (ver Pre-requisitos).
   - **Uno** → ese es el base.
...
2. Leer `perfil/profile.md` — trae material que el CV base pudo haber dejado afuera (una tech secundaria, un logro, un proyecto) y que **es relevante a ESTA JD**. Acá está la munición para subir el match sin inventar.
3. Leer el método antes de reescribir (no se duplica — vive en cv-builder):
   ...
   - `.claude/skills/cv-builder/references/typst-template.md` — API del package `silver-dev-cv` y el formato exacto de las funciones.
...
1. Escribir la copia tailoreada como **archivo nuevo** (nunca pisar el base):
   `trabajo/cv/{slug}-cv-{jd-slug}.typ`
...
2. Compilar:
   ```bash
   typst compile trabajo/cv/{slug}-cv-{jd-slug}.typ trabajo/cv/{slug}-cv-{jd-slug}.pdf
   ```
...
4. Copiar el PDF a `~/Downloads/` con nombre legible:
   ```bash
   cp trabajo/cv/{slug}-cv-{jd-slug}.pdf "$HOME/Downloads/{Nombre} - CV - {Rol o Empresa}.pdf"
   ```
...
1. **Path del PDF** (dentro de `trabajo/cv/`) + que quedó en Downloads con el nombre legible.
...
- **2026-09-07** — **Nunca pisar el CV base.** La salida SIEMPRE es un archivo nuevo `trabajo/cv/{slug}-cv-{jd-slug}.typ`. El base (`{slug}-cv.typ`) es el material validado y no se toca.
...
- **2026-09-07** — **Checks de render heredados de cv-builder:** 1 página; fechas exactas (mes+año); `#oneline-title-item` sin `:` final; escapar `@ ~ $ #` — escapar con `\` o reformular.
...
- **No hay CV base** — `cv-tailor` no genera desde cero. Derivar a `cv-builder`, y una vez validado el base, volver con la JD.
...
- **El alumno quiere el CV en otro idioma** — si el base existe en ese idioma, tailorear sobre ese; si no, primero generar el base en ese idioma con `cv-builder`.
