---
name: cv-builder
description: Genera el CV base en PDF del alumno de CoderHub a partir de su perfil (perfil/profile.md), usando el template Typst silver-dev-cv. Opcionalmente lo tailorea ligero a una oferta al momento de crearlo — sin inventar nada. Triggers on "armame el CV", "generá mi CV", "hacé mi currículum", "creá mi CV base", "/cv-builder", "necesito un CV en PDF". Lee el perfil que arma coderhub-setup; si no existe, deriva a coderhub-setup. Requiere typst instalado (brew install typst). NO inventa experiencia — el perfil es la única fuente de verdad. Para adaptar un CV YA validado a una oferta específica (exagerando el fit, reordenando todo hacia la JD), usá `cv-tailor`, no esta skill.
---

# CV Builder

Arma el CV en PDF del alumno desde su `perfil/profile.md`, con el template Typst `silver-dev-cv`. Si el alumno pega una oferta, adapta el orden y el énfasis para esa oferta — sin inventar experiencia.

## Pre-requisitos

- **`perfil/profile.md` lleno** (corrió `coderhub-setup` antes). Si está sin llenar → *"No encuentro tu perfil cargado. Corré primero `coderhub-setup` y volvé."* y parar.
...
- Un `.typ` generado con los datos del alumno en `trabajo/cv/`.
- El CV compilado en PDF: `trabajo/cv/{slug}-cv.pdf` (+ copia en `~/Downloads/`).
...
- **Upstream:** `coderhub-setup` — genera el `perfil/profile.md` que esta skill consume. Sin perfil, esta skill no corre.
- **Sibling:** `cv-tailor` — adapta un CV YA validado a una oferta puntual, yendo más fuerte con el match. Si el alumno ya tiene CV base y quiere una versión para una oferta, es `cv-tailor`.
...
1. Chequear `perfil/profile.md`. Si está sin llenar, derivar a `coderhub-setup` y parar.
2. Chequear `typst`:
   ```bash
   which typst
   ```
...
1. Leer `perfil/profile.md` entero.
...
1. El directorio `trabajo/cv/` ya existe en el repo (si no, `mkdir -p trabajo/cv`).
2. Por cada idioma decidido en Step 2 (uno o dos), escribir un `.typ` en `trabajo/cv/` siguiendo el formato exacto de las funciones del package (ver reference). Usar `@preview/silver-dev-cv:1.0.2`. Naming:
...
3. Compilar cada `.typ` a su PDF con el mismo nombre:
   ```bash
   typst compile trabajo/cv/{slug}-cv.typ trabajo/cv/{slug}-cv.pdf
   ```
...
5. Copiar el/los PDF a `~/Downloads/` como conveniencia:
   ```bash
   cp trabajo/cv/{slug}-cv*.pdf ~/Downloads/
   ```

## Step 5 — Confirmar + ofrecer ajustes

Mostrar:
1. **Path del PDF** (dentro de `trabajo/cv/`) + que quedó copiado en Downloads.
...
- "adaptalo a esta otra oferta: {...}" → tailoreo a ese puesto (cv-tailor)
...
- **El perfil está muy incompleto** — si faltan bloques clave (experiencia, stack, logros), el CV sale pobre. Derivar a `coderhub-setup` para completar el perfil antes de insistir con el CV.
