# Perfil

> Lo llena `configurar-coderhub` la primera vez. Después, todas las skills lo leen.
> Para actualizarlo (nuevo logro, cambio de stack, banda salarial), editá los campos o re-corré `/configurar-coderhub`.

---

## Identidad

- **Nombre completo:**
- **Email:**
- **Teléfono / WhatsApp:** (con código de país, formato `+5491153190688`)
- **LinkedIn:** (URL completa, sin trailing slash)
- **Portfolio:** (URL si tenés)
- **GitHub:** (URL si tenés)
- **Otros links:** (certificaciones, blog, etc.)

## Ubicación + legales

- **País de residencia:**
- **Ciudad:**
- **Nacionalidad:**
- **Modalidad preferida:** (remoto / híbrido / presencial)
- **Restricciones legales:** (ej. "puede trabajar como contractor sin restricciones, visa requerida para EU")

## Idiomas

- **Español:** (nivel)
- **Inglés:** (nivel — ej. B2 conversacional, C1, native)
- **Otros:**

---

## Rol y experiencia

- **Rol actual:** (puesto + empresa, ej. "Senior Backend Engineer @ Spotlio")
- **Años de experiencia:**
- **Salario actual:** (USD/mes o EUR/mes según corresponda)

### Roles previos relevantes

- (lista breve, ej. "Full Stack Dev @ Acme — 2 años")
- ...

---

## Stack técnico

### Stack principal (lo que más sabés y querés vender)

- (ej. PHP, Laravel, Vue.js, Python, Django)

### Stack secundario (lo que tocás pero no es el foco)

- (ej. Node.js, MongoDB, Angular)

### Diferenciadores técnicos

- (ej. agentes IA con Claude, testing automatizado con Pest/Playwright, arquitectura de microservicios)

### Educación

- (instituciones + títulos + fechas)

### Certificaciones destacadas

- (las más relevantes, no listar todas)

---

## Logros concretos (con números)

> Oro para LinkedIn / CV / entrevistas. Cada logro con un número, un porcentaje o un resultado medible.

- (ej. "Reduje 70% los costos de generación de IA — de $10 a $2-3 USD por guía")
- (ej. "Migré sistema legacy MS-DOS a Django — automaticé proceso manual de 3 días a 1 minuto")
- ...

---

## Objetivo de búsqueda

- **Rol target:** (ej. Senior Backend, Tech Lead, Full Stack Engineer remote)
- **Banda salarial objetivo:**
  - Piso: USD/mes
  - Realista: USD/mes
  - Stretch: USD/mes
- **Mercados target:** (ej. Argentina, España, LATAM, Global remote)
- **Empresas tipo (descartá lo que NO querés):** (ej. "producto, no consultora intermediaria")
- **Plazo deseado:**

---

## Modo confidencialidad

- **Estás actualmente empleado:** (sí / no)
- **Tu empresa actual debe enterarse de la búsqueda:** (sí / no)
- **Modo stealth requerido:** (sí / no)

> Empleado + la empresa no debe saberlo → stealth ON. Las skills evitan likes/comentarios en posts de hiring y no activan "Open to Work" público.

---

## Bloqueos diagnosticados (lo que CoderHub identificó en tu call)

> Sale de la transcripción de tu call de discovery. Sin transcripción, dejalo vacío y corré `/configurar-coderhub` después de la llamada.

- (ej. "El bottleneck es no-técnico: pasa HR pero rebote pre-técnica")
- (ej. "LinkedIn anonimizado te elimina de búsquedas de recruiters")
- ...

## Anti-distracciones (lo que NO hay que hacer)

> Trampas mentales típicas que el equipo identificó. Las skills las respetan para mantener foco.

- (ej. "No desviarse a aprender inglés conversacional antes — el target $4-5K en mercado hispano no lo necesita")
- (ej. "No aprender Next.js u otra tech nueva — el desafío es de posicionamiento")
- ...

---

## Mentor asignado

- **Coach principal:**
- **Mentor técnico (System Design):** (Ariel u otro si aplica)
- **Fecha de arranque del programa:** YYYY-MM-DD

---

*Última actualización: (fecha — la actualiza configurar-coderhub automáticamente)*
